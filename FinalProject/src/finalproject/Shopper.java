...
public class Shopper extends User{
	 ...
	 */
	public ArrayList<String> addtoCart(Product product,int backorder){
		ArrayList<String> result = product.checkAvailableQuantity(backorder);
		if (!result.isEmpty()){
	...
	public void addInvoice(){
		...
		for (Map.Entry<Product, Integer> cart : cart.entrySet())
		{
		    Product p = cart.getKey();
		    total_price += p.price*cart.getValue();
		}
		Invoice i = new Invoice(cart,total_price);
		this.invoice.add(i);
	}
	...
	public String iToString() {
		...
		for (Invoice i: invoice){
			...
			for(Map.Entry<Product,Integer> ca: cart.entrySet()){
				Product p = ca.getKey();
